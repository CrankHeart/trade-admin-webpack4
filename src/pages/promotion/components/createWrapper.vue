        ...
        o.spu = Object.keys( brandData ).filter( v => v !== 'name' ).map( val => {
          ...
          if ( spu.type ) {
            spu.sku = Object.keys( spuData ).filter( v => v !== 'name' ).map( v => {
              let { id, name, specification, price, lcSkuCode, settlePrice, bottomPrice, platformPrice, salesLimitNum, salesPrice, level1Price } = spuData[ v ]
              return {
                id,
                price, lcSkuCode, settlePrice, bottomPrice, platformPrice, salesLimitNum: +salesLimitNum, salesPrice, specification, level1Price,
