    ...
    >
      <template slot-scope="{row,con}">
        <template v-if="con.prop === 'control'">
          <el-button @click="openWin(row)" type="primary" size="small">详情</el-button>
        </template>
...
export default {
  ...
  computed: {
    ...
    conditionByOrderType() {
      let arr = [];
      arr.push( { value: "", key: "orderSn", label: "订单号", type: "text" } );
      arr.push( {
        value: [],
        key: "orderDate",
        label: "订单创建时间",
    ...
    configByOrderType() {
      let arr = [
        { prop: "amount", label: "订单金额", width: "220px" },
      ...
      arr.unshift( { prop: "orderSn", label: "订单号", width: "220px" } );
      return arr;
    }
  },
  methods: {
    ...
    openWin( row ) {
      ...
      openNewWebWin( `order/detail/${row.orderSn}`, "orderSn" + row.orderSn || "" )
    },
    disabledByChecked( row ) {
      return this.selectedSnLists.indexOf( row.orderSn ) == -1
    },
    selectionChange( items ) {
      this.selectedLists = items
      this.selectedSnLists = items.map( item => item.orderSn )
    },
    invoiceCtr( item ) {
    ...
    getSelectedLists() {
      ...
      let ret = this.lists.filter( item => {
        return this.selectedSnLists.indexOf( item.orderSn ) !== -1
      } )
      return ret
    },
    loadData() {
      ...
      this.$http( API[ "getPaymentVoucherOrders" ], {
        ...params,
