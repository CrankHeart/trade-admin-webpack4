<template>
  <div class="account-invoice-header" ref="createAccountInvoice">
    <template v-if="hadInvoice">
      <div class="title">
        <h3>开票信息</h3>
        <span>
          ...
          <el-row>
            ...
            >
              ...
              <div v-else>
                <span>{{ msg.label }}：</span>
                <span
                  v-if="msg.prop === 'invoiceKind'"
                >{{formatInvoiceKind(invoiceMsg[ msg.prop ])}}</span>
                <span
    ...
    <template v-else>
      <div style="text-align:center;padding:15px;">暂无发票信息</div>
    </template>
  </div>
</template>
...
import permission from '@/pages/server/permissions/accountInvoiceHeaderPermissions'
import invoiceList from '@/pages/invoice/invoiceList.vue'
import { Loading } from 'element-ui'
@Component( {
  name: 'order-invoice-header',
  components: {
    invoiceList
  },
  mixins: [ permission ],
...
export default class AccountInvoiceHeader extends Vue {
  ...
  @Prop( { default: false } ) hadInvoice: boolean // 是否已经申请过发票
