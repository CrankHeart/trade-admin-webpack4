<template>
  <!-- 换货记录 serviceType=2 服务单类型（1-退货，2-换货，3-退款）-->
  <div class="exchangeRecord">
    ...
    >
      <template slot-scope="{row,con}">
        ...
        <template v-else-if="con.prop === 'logisticInfo'">
          <template
            v-if="(row.returnLogisticInfoList && row.returnLogisticInfoList.length) || (row.exchangeLogisticInfoList && row.exchangeLogisticInfoList.length)"
          >
            <template v-if="row.returnLogisticInfoList && row.returnLogisticInfoList.length">
              <div>退货：{{ row.returnLogisticInfoList[0].companyName || "暂无" }}</div>
              <div>
                <el-button
            ...
            <template v-if="row.exchangeLogisticInfoList && row.exchangeLogisticInfoList.length">
              <div>换货：{{ row.exchangeLogisticInfoList[0].companyName || "暂无" }}</div>
              <div>
                <el-button
                  v-if="$permissionValidate(1096)"
                  type="text"
                  @click="logicShow(row.exchangeLogisticInfoList)"
                >查看包裹信息</el-button>
              </div>
            </template>
          </template>
    ...
    <el-dialog class="logistic" v-model="logisticVisible" title="物流信息">
      <el-row>
        <el-col :span="3">共{{ logistic.length }}个包裹：</el-col>
      </el-row>
...
import { IDName, IBaseConfig, exchangeRecordBaseConfig, serviceType, serviceStatus, findItem, times } from "./status";
...
export default class ExchangeRecord extends Vue {
  ...
  baseConfig: Array<IBaseConfig> = exchangeRecordBaseConfig;
